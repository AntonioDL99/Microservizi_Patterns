package com.patterns.webflux.sec03.util;

import com.patterns.webflux.sec03.dto.InventoryRequest;
import com.patterns.webflux.sec03.dto.OrchestrationRequestContext;
import com.patterns.webflux.sec03.dto.PaymentRequest;
import com.patterns.webflux.sec03.dto.ShippingRequest;
...
public class OrchestrationUtil {
    ...
    private static void buildPaymentRequest(OrchestrationRequestContext ctx) {
        var paymentRequest = PaymentRequest.create(ctx.getOrderRequest().getUserId(),
                ctx.getProductPrice() * ctx.getOrderRequest().getQuantity(), ctx.getOrderId());
        ctx.setPaymentRequest(paymentRequest);
    }

    private static void buildInventoryRequest(OrchestrationRequestContext ctx) {
        var inventoryRequest = InventoryRequest.create(ctx.getOrderId(), ctx.getOrderRequest().getProductId(),
    ...
    private static void buildShippingRequest(OrchestrationRequestContext ctx) {
        var shippingRequest = ShippingRequest.create(ctx.getOrderRequest().getQuantity(),
                ctx.getOrderRequest().getUserId(), ctx.getOrderId());
